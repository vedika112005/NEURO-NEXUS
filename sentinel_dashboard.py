...
# ==========================================
# 🛰️ LIVE TRAFFIC SIMULATOR (Background Thread)
# ==========================================
...
MOCK_USERS = ["user_01@company.com", "user_02@company.com", "user_05@company.com", "admin@company.com"]

def traffic_generator_loop():
    """Background loop to generate realistic traffic to the CSV."""
    live_file = "live_stream_data.csv"
    
    # Initialize file if not present
    if not os.path.exists(live_file):
        pd.DataFrame(columns=['timestamp', 'sender', 'subject', 'status', 'flag_reason']).to_csv(live_file, index=False)

    while True:
        sender = random.choice(MOCK_USERS)
        if random.random() < 0.15: # 15% Risk
            scenario = random.choice(RISK_SCENARIOS)
            subject, status, reason = scenario["subject"], "CRITICAL", scenario["reason"]
        else:
            subject, status, reason = random.choice(SAFE_SUBJECTS), "NORMAL", "-"
        ...
        # Append without lock (single writer, multiple readers is usually fine for CSV append)
        new_row.to_csv(live_file, mode='a', header=False, index=False)
        time.sleep(random.uniform(2.0, 4.0))

@st.cache_resource
def start_traffic_engine():
    """Starts the traffic generator thread exactly once."""
    thread = threading.Thread(target=traffic_generator_loop, daemon=True)
    thread.start()
    return True

# Initialize Traffic Engine
start_traffic_engine()
